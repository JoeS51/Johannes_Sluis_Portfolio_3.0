...
I wanted a single, reliable worker that listened to Azure Service Bus and fanned out transactional emails (welcome emails, weekly reports, and alerts). Cron-based SMTP jobs were brittle, so I moved to an event-driven approach that could scale horizontally and survive transient cloud hiccups.
...
I used the official Azure SDK plus Tokio’s concurrency primitives. The worker prefetches messages in small batches, processes them concurrently with a cap, and completes or dead-letters explicitly.
...
- Idempotency keys combine \`tenant_id\`, \`event\`, and \`recipient\`, so duplicate messages short-circuit gracefully.
